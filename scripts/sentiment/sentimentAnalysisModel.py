...
# plot the model performance graphs (changes based on if it is loss VS accuracy and stock prediction VS sentiment analysis
...
def train_sentiment_analysis_model():
  ...
  # Sequencing: Sequences of numbers (tokenized words) to represent sentences
  ...
  model = Sequential([
    ...
    Dropout(0.25),  # dropout and regularizer to prevent overfitting and too high weights
    Dense(1, activation='sigmoid')
])
  optimizer = Adam(learning_rate=0.01)
  model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])

  # learning rate scheduler to improve accuracy/loss convergence
  def lr_scheduler(epoch, lr):
      if epoch < 80 and epoch > 15 and epoch % 10 == 0:  # Reduce learning rate after 100 epochs for better convergence
          return lr * 0.5
      return lr
  scheduler = LearningRateScheduler(lr_scheduler)

  # train the model and plot the accuracy/loss graphs
  history = model.fit(training_padded, training_labels, epochs=150,  batch_size=10, validation_data=(testing_padded, testing_labels), verbose=2, callbacks=[scheduler])

  if testing:
    plot_graphs(history, "accuracy", "../public/sentiment_accuracy.png", "Sentiment Analysis")
    plot_graphs(history, "loss", "../public/sentiment_loss.png", "Sentiment Analysis")
