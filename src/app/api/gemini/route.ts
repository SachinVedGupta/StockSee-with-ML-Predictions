...
export async function POST(req: Request) {
  ...
  try {
    ...
    const eventTerms = ["launch*", "unveil*", '"product release"', "CEO", '"chief executive"', "resign*", "appoint*", "layoff*", "hiring", "hire*", "acquir*", "restructur*", "earnings", "guidance", "acquisition", "merger", "partnership", "contract", "recall", "lawsuit", "regulator*", "tariff*", "approval", '"interest rate"'];
    ...
    const mentionsCompany = (text: string) => companyTerms.some(term =>
      ` ${text.toLowerCase().split(/[^a-z0-9]+/).join(" ")} `.includes(` ${term.toLowerCase().split(/[^a-z0-9]+/).join(" ")} `));
      ...
      await Promise.all(Array.from(new Set<string>(dates)).map(async date => {
        ...
        try {
          ...
          const articles = Array.isArray(body.data) ? body.data.filter((item: any) =>
            ...
            typeof item.title === "string" && mentionsCompany(item.title) && typeof item.published_at === "string" &&
    ...
    ).slice(0, 30).map((move: any) => ({ date: move.date, changePercent: move.changePercent, measuredMove: `${move.changePercent >= 0 ? "ROSE" : "FELL"} ${Math.abs(move.changePercent).toFixed(1)}% over 20 trading observations` })) : [];
...
The measuredMove field is authoritative. A source may discuss a single-day rise or fall that differs from this 20-observation move. NEVER substitute that daily direction for the measuredMove. For a measured rise, negative news is a possible headwind, not evidence that the highlighted stock move was a decline; for a measured fall, positive news is a possible offset. Describe the event and its business implications without asserting that it explains a price direction contradicted by measuredMove.
Use ONLY the supplied sources from the 14-day lead-up window through each highlight date. Their date field identifies the chart highlight; publishedDate is the article publication date, not necessarily the event date. Events a few days earlier may be relevant. Consider product launches, earnings, guidance, competitive or regulatory developments, and broader sector or macroeconomic conditions WHEN supported by the sources. Prioritize actual company developments: a named product launch or delay, a CEO appointment or departure, layoffs or significant hiring, acquisitions, earnings above or below expectations, changed guidance, major deals, recalls, lawsuits, or regulatory decisions. Prefer these over analyst ratings, fund holdings, insider trades, and daily price recaps. Only call earnings a beat or miss if the source explicitly compares them with expectations. Do not treat an executive's unrelated companies or personal lawsuits as events at this company. Competitor news and passing mentions alone are not enough. Omit a highlight if none of its sources describes a concrete development affecting this company or its operations. Do not assume a generic analyst forecast or an unrelated article explains the move.
