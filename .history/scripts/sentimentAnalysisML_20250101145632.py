...
def sentiment_from_sentences(sentences):
  ...
  # test out the model with input sentences (closer to 0 means negative sentiment AND closer to 1 means positive sentiment)
  sentences = ["the company had sales increase by 10%", "falling", "Analysts predict a decline in AMD's stock price over the next year due to increased competition, market uncertainties, and potential slowdowns in consumer demand for semiconductor products."]
  sequences = tokenizer.texts_to_sequences(sentence)
  padded = pad_sequences(sequences, maxlen=max_length, padding='post', truncating='post')
