...
def with_sentiment_ml_to_predict(ticker_symbol): # returns predictions made using both Close price and Sentiment Score as an input
    ...
    if trainSetting == 1: # train a model based on just the previous stock's data
        ...
        # model.save('./sentiment_storage/DOW_Trained_stock_tf_model.keras.keras')
        ...
        model.compile(loss='mse', optimizer=Adam(learning_rate=0.0002)) # recompile with lower learning rate to prevent overfitting to this new dataset
        ...
        plt.figure(figsize=(10, 6))
        the_title = f"Loss Function (MSE) for pretrained (on the 30 stocks in DOW JONES) prediction model"
        plt.plot(r.history["loss"])
        plt.plot(r.history["val_loss"])
        plt.title(the_title)
        plt.xlabel("Epochs")
