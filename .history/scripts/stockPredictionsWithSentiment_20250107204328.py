...
def with_sentiment_ml_to_predict(ticker_symbol): # returns predictions made using both Close price and Sentiment Score as an input
    ...
    train = False
    trainSetting = 0 
        # 0 for using existing multi-trained model (trained on the 30 Dow Jones stocks)
        # 1 for training based on just the entered stocks historical data
        # 2 for adding to multi-trained model and enhancing it by training it 
    ...
    # Train the model or load in existing one
    if train:
