...
export async function fetchStockNews(ticker: string, date: []) {
  ...
  const prompt = `Tell me the reason (an event) why ${ticker} stock price shifted on ${date} in the past. Give event, how it will impact future stock price on one continuous line.`;
