...
export async function fetchStockNews(ticker: string, date: []) {
  ...
  const prompt = `Tell me the reason (an event) why ${ticker} stock price shifted on ${date} in the past. Give what event happened that day. Be in present tense and say the future impact the event will have on the companies stock. The date and the source url. Everything for a specific day must be on one continuous line.`;
